...
struct FootballTeam: Codable {
    ...
    func toCommonTeam()-> CommonTeam {
        return CommonTeam(team_key: self.team_key, team_name: self.team_name, team_logo: self.team_logo ?? "")
    }
}
...
struct TennisPlayer: Codable {
    ...
    func toCommonTeam()-> CommonTeam {
        return CommonTeam(team_key: self.player_key, team_name: self.player_name, team_logo: self.player_logo ?? "")
    }
}
